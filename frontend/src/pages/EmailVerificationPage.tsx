...
const EmailVerificationPage: React.FC = () => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      const msg = error?.response?.data?.message ?? "An error occurred while resending the email.";
        ...
        <div className="w-full md:w-1/2 flex items-center justify-center p-10 bg-white/95">
          <div className="w-full max-w-md bg-white p-10 rounded-3xl shadow-2xl">
            ...
            <p className="text-center text-lg mt-4">
              A verification email has been sent to <strong>{email || "your email"}</strong>. Please check your inbox (and spam folder)!
            </p>
