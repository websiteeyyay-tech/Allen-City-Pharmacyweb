// src/pages/pharmacist/Settings.tsx
...
const PharmacistSettings: React.FC = () => {
  ...
  const [email, setEmail] = useState("pharmacist@email.com");
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const res = await api.get("/pharmacist/settings/1"); // Example: pharmacistId = 1
        const data = res.data;
        setDarkMode(data.darkMode ?? false);
        setNotifications(data.notifications ?? true);
        setName(data.name ?? "Pharmacist Name");
        setEmail(data.email ?? "pharmacist@email.com");
  ...
  const handleSave = async () => {
    try {
      await api.post("/pharmacist/settings", {
        name,
        email,
        password,
        darkMode,
      ...
      >
        ...
        <div className="flex space-x-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleSave}
            className="flex items-center bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white px-4 py-2 rounded-xl shadow-lg transition-all"
          >
          ...
          <div className="flex flex-col items-center mt-3 space-y-3">
            <div className="relative group">
              <img
                ...
                alt="Profile"
                className="w-32 h-32 rounded-full object-cover border-4 border-green-200 shadow-md group-hover:scale-105 transition-transform"
              />
              <button
                onClick={() => fileInputRef.current?.click()}
                className="absolute bottom-1 right-1 bg-green-600 p-2 rounded-full text-white shadow-lg hover:bg-green-700 transition"
              >
            ...
            <div>
              ...
              <input
                ...
                className={`w-full rounded-xl p-2.5 border focus:ring-2 focus:outline-none ${
                  darkMode
                    ? "bg-gray-900 border-gray-700 text-gray-200 focus:ring-green-500"
                    : "bg-white border-gray-300 text-gray-800 focus:ring-green-400"
                }`}
              />
            </div>
